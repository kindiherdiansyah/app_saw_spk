...
include_once '../layouts/head.php'; 
  ...
  $query = "SELECT tbl_himpunan.id_kriteria, tbl_himpunan.namahimpunan,tbl_himpunan.nilai,tbl_himpunan.keterangan, tbl_kriteria.nama_kriteria 
FROM tbl_himpunan INNER JOIN tbl_kriteria ON tbl_himpunan.id_kriteria=tbl_kriteria.id WHERE tbl_himpunan.id=".$id;
              ...
              <div class="col-md-12">
                ...
                <input type="hidden" name="id" value="<?=$_GET['id']?>">
                  ...
                  while ($data = mysqli_fetch_array($result))
                    {?>
                  <option value="<?=$data['id']?>" <?=($data['id']==$row['id_kriteria']) ? 'selected' : '';?>><?=$data['nama_kriteria']?></option>
                   <?php }
                  ?>
                  <option>tes</option>
                  </select>
